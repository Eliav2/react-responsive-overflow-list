...
import { Popover, PopoverContent, PopoverTrigger } from "@/components/shadcn-base-ui/popover";
...
interface OverflowListChildrenProps extends Omit<PrimitiveChildrenProps, "renderOverflow" | "renderOverflowProps"> {
  overflowLabel?: (count: number) => string;
  popoverContentClassName?: string;
  items?: never;
  renderItem?: never;
...
export const OverflowList = React.forwardRef(function OverflowList<T>(
  ...
  const {
    ...
    ...primitiveProps
  } = props as OverflowListProps<T> & {
    renderOverflowItem?: (item: T, index: number) => React.ReactNode;
    renderOverflowContent?: (hiddenItems: T[]) => React.ReactNode;
  };
  const renderItem = "renderItem" in primitiveProps ? primitiveProps.renderItem : undefined;
...
const OverflowPopover = React.forwardRef(function OverflowPopover<T>(
  {
    items,
    label,
    renderItem,
